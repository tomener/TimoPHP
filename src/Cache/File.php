...
class File
{
     ...
     * @return bool
     */
    public function clearExpired($path = '')
    {
        $sub_dir = !empty($this->options['sub_dir']) ? DS . $this->options['sub_dir'] . DS : '';
        $path = !empty($path) ? $path : ($this->options['path'] . $sub_dir);

        $is_rmdir = true;
        foreach (glob($path . '*') as $file) {
            if (is_dir($file)) {
                $ret = $this->clearExpired($file . DIRECTORY_SEPARATOR);
                if ($is_rmdir === true && $ret === false) {
                    $is_rmdir = false;
                }
                continue;
            }
            ...
            if (false !== $content) {
                $expire = (int)substr($content, 8, 12);
                if (0 != $expire && time() > filemtime($file) + $expire) {
                    //缓存过期删除缓存文件
                    $this->unlink($file);
                } else {
                    $is_rmdir = false;
